[EntityEditorProps(category: "GameScripted/CTI", description: "CTI USSR Factorys")]
class SCR_CTI_FactorysUSSR
{
	// USSR factorys
	FactionKey factionKey = "USSR";
	ref array<ref SCR_CTI_FactoryData> g_USSR_Factorys = new array<ref SCR_CTI_FactoryData>;
	...
	int findIndexFromResourcename(ResourceName resName)
	{
		...
		for (int i = 0; i < g_USSR_Factorys.Count(); i++)
		{
			factoryData = g_USSR_Factorys[i];
