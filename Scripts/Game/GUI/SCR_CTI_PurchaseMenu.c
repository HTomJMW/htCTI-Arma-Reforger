class SCR_CTI_PurchaseMenu : ChimeraMenuBase
{
		...
		if (m_timeDelta > TIMESTEP)
		{
			...
			if (currentSelectedFactoryTypeIcon != m_buttonEventHandler.getSelectedFactoryTypeIcon())
			{
				...
				if (m_listboxcomp.GetItemCount() > 0 && selectedId > -1 && m_listboxcomp.GetElementComponent(selectedId) && m_listboxcomp.IsItemSelected(selectedId))
				{
					...
					if (!m_gunner.IsEnabled() && hasVehicleTurret(unitData.getResname()))
					{
						m_gunner.SetEnabled(true);
						m_gunner.SetColor(SCR_CTI_Constants.CTI_ORANGE);
					}
				} else {
					m_cost.SetText("Cost:");
					
					if (m_gunner.IsEnabled())
					{
						m_gunner.SetEnabled(false);
						m_gunner.SetColor(Color.Gray);
					}			
	...
	//------------------------------------------------------------------------------------------------
	bool hasVehicleTurret(ResourceName resName)
	{
		...
		for (int c = components.Count() - 1; c >= 0; c--)
		{
			...
			if (meshComponent.GetClassName() == "SCR_EditableVehicleComponent") break;
        ...
        if (meshComponent)
        {
			...
			array<int> values = {};
			infoContainer.Get("m_aOccupantFillCompartmentTypes", values);

			if (values.Contains(ECompartmentType.Turret)) return true;
		}
