...
class SCR_CTI_UpgradeComponent : ScriptComponent
{
	...
	void update()
	{
		m_elapsedTime = m_gameMode.GetElapsedTime();
		m_timeDelta = m_elapsedTime - m_lastTime;
	...
	{
		m_gameMode = SCR_CTI_GameMode.Cast(owner);
	...
	{
		if (!m_rplComponent.IsProxy()) GetGame().GetCallqueue().CallLater(update, 5 * 1000, true);
	}
